...
function build_action_space(N::Int64)
	...
	# Phase shifter
    for n in range(1,N) 
        push!(actions,action(PS,[n],round(pi/8,digits=r)))
    end
	# Beam-splitter
    for i in range(1,N) 
        for j in range(i+1,N)
            push!(actions,action(BS,[i,j],round(1.5,digits=r)))
        end
    end
	# Single-mode squeezer
    for n in range(1,N) 
        push!(actions,action(SMS,[n],round(0.8,digits=r)))
    end
	# Two-mode squeezer
    for i in range(1,N)
        for j in range(i+1,N)
            push!(actions,action(TMS,[i,j],round(0.8,digits=r)))
        end
    end
    return actions
end
...
function Env(N::Int64,n_circuit::Int64) #Constructor
    start_circuit = Vector{action}(undef,0) #initial circuit
    circuit = Vector{action}(undef,0) #initial circuit also
    params = Vector{Float64}(undef,0)
    lower = Vector{Float64}(undef,0)
    upper = Vector{Float64}(undef,0)
    actions = build_action_space(N)
    return Env(circuit,N,actions,start_circuit,n_circuit,params,lower,upper,0.0)
end
