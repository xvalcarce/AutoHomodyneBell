...
function Custom_Env(N::Int64,n_circuit::Int64) #Constructor
    ...
    return Custom_Env(circuit,N,actions,start_circuit,n_circuit,params,lower,upper,0.0)
end
...
function build_action_space(N::Int64)
    ...
    #here we discretize the gates
    for n in range(1,N) 
        for k in 0:10
            push!(actions,node(PS,[n],round(0.1 + k/10*(pi-0.1),digits=r)))
        end
    end
    ...
    for i in range(1,N) 
        for j in range(i+1,N)
            for k in 1:7
                push!(actions,node(BS,[i,j],round(k*pi/4,digits=r)))
            end
        end
    end
...
function optimize_herald!(env::Custom_Env)
    ...
    function f(x)  #penalty for evaluating function outside of range and if the violation drops below 2.068
        ...
        if abs(CHSH(env,x))< 2.0676
            return exp(abs(2.0676-CHSH(env,x)))
        end
...
#=
# this is the function defined by Corentine

function RLBase.reward(env::Custom_Env) 
    k1=10 #multiplication coefficient 
    k2=100
    if env.chsh_absvalue<1e-3  #this happens if the optimization of the parameters failed, see where optimize_params! is called
        return -10
    elseif env.chsh_absvalue<2
        return k1*env.chsh_absvalue
    else
        return k2*env.chsh_absvalue   
    end 
